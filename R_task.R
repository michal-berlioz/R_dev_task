...
Modifier <- function(input.data,
  ...
  for (variable in var.names){
    ...
    for (variant in c(1:variants.count)){
      new.var.name <- paste(substr(variable, 1, 2), str_pad(s, 2, pad = "0"), substr(variable, 3, 10), sep = "")
      output.data[new.var.name] <- c(stats::filter(output.data[variable], c((s/100)), method = "recursive")) #poz. na liscie wspolczynnikow = rzad opoznienia
      s <- s + step  
    }
  }
  ...
  output.data[num.columns] <- apply(output.data[num.columns], 2, function(x) formatC(x, format = "f", drop0trailing = TRUE, decimal.mark = dec))
